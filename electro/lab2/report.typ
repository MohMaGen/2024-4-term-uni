...
+ Графики
    ...
    #figure(
        caption: "Сравнение графиков расчётных и эксперементальных данных.",
        image("./plot3.png")
    )
    
    Из графиков видно, что данные расчётные данные совподают с эксперементальными
    в пределе допустимой погрешности.

== Вывод
    В ходе лабораторной работы я расчитал цепи @fig-2-1 и @fig-2-2 при заданных частотах
    источника. В результате сопротивление конденсатора обратно пропорционально частоте,
    а сопротивление катушки прямо пропорционально частоте. В результате графики эксперементальных
    данных достаточно точно совпали с графиками рассчётных данных.

    Графики сопративления катушки и конденсатора пересекаются в точке $~50, "Гц"$. В этой точке
    происходит резонанс сопративлений ($X_L = X_C$). Таким образом точка резонанса находится
    около 50 Гц.
