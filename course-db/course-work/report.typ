...
== Текущии компания поставщик автомобилей.

Так как информация о компании поставщике не привязана к конретному автомобилю,
	а к заказу (автомобили могут быть перепроданы между компаниями), то
	тукющая компания поставщик автомобиля вычисляется по последнему заказу
	с этим автомобилем.

Для удобства я содзад представление показывающие последнии заказы кажого 
	автомобиля:
...
#figure(
	kind: table,
	caption: [Последнии заказы всех автомобилей.],
	tablex(
		columns: range(7).map(it => auto),
...
В начале я создал представление отображающие количество автомобилей
    за каждый месяц (отображение считате месяцы, в которых не было заказов,
    в том чиле).
...
Для нахождения среднего количесва автомобилей по месяцам, достаточно выполнить
запрос:
```sql
