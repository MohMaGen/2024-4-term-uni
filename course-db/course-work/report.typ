...
Запрос отображающий все заказы с подключением родительских таблиц.
...
== Компания поставщик автомобиля

Представление ``` last_orders_of_cars``` показывает для каждого
	автомобиля последний заказ (автомобили, которые ещё не заказывали
	не перечисляются).

#raw(read("./last_orders_of_cars.sql"), block: true, lang: "sql")

Информация о последних заказов для каждого автомобиля. 

#raw(read("./last_orders_of_cars_exp.sql"), block: true, lang: "lang")
...
Функция ``` supplier_of_car``` определяет поставщика по VIN номеру автомобиля. 
	Функция принимает ``` VIN``` номер автомобиля как входной параметр и
	возвращает ``` ID``` поставщика, который последний раз предоставлял
	этот автомобиль для брони.

#raw(read("./req2.sql"), block: true, lang: "sql")

Запрос выводящий все автомобили и соответствующих им поставщиков. 
...
== Запросы для определения, какие автомобили находятся на заказе, а какие стоят в гараже.

Функция ``` ordered_cars``` принимает дату, как аргумент, и возвращает 
	автомобили, которые находились на заказе в эту дату.

Функция ``` cars_on_parking``` принимает дату, как аргумент, и возвращает
	автомобили, которые находились в гараже в эту дату.

Функция ``` cars_state``` принимает дату, как аргумент, и возвращает
	таблицу автомобилей, с информацией о из состояния (находится ли
	автомобиль на заказе или стоит в гараже).

#raw(read("./req3.sql"), block: true, lang: "sql")

Запрос для получения состояния автомобилей на 5-е января 2023 года.
#raw(read("./req3_show.sql"), block: true, lang: "sql")
...
#figure(
	...
	tablex(
		columns: range(3).map(it => auto),
		..csv("./res3_show.csv").flatten()
	)
)

== Среднее количество выданных автомобилей за месяц

Представление ``` ordered_cars_by_monthes``` показывает сколько было выдано
	автомобилей в каждом месяце с первого заказа по последний.

#raw(read("req4.sql"), block: true, lang: "sql")


Запрос, чтобы получить количество выданных автомобилей в каждом месяце.
#raw(read("./req4_show.sql"), block: true, lang: "sql")
...
#figure(
	...
	tablex(
		...
		..range(3).map(it => ([Год], [Месяц], [Кол.])).flatten(),
		..csv("./res4_show.csv").flatten()
	)
)
...
== Функции и запросы для получения заказов клиента.

Функция ``` orders_of_client``` принимает ID клиента и возвращает все
	заказы этого клиента:

#raw(read("./req5.sql"), block: true, lang: "sql")

Пример использования, для нахождения всех заказов Краснова Дмитрия 
	Александровича:

#raw(read("./req5_show.sql"), block: true, lang: "sql")
...
#figure(
	...
	tablex(
		align: horizon,
		columns: range(5).map(it => auto),
		..csv("./res5_show.csv").flatten()
	)
)

== Запросы выдающие все автомобили взятые клиентом

Функция ``` cars_of_client``` принимает ID клиента и возвращает все 
машины, которы клиент заказывал.

Функция ``` cars_of_client_by_range``` принимает ID клиента, а также промежуток
	даты (начало и конец промежутка), и возвращает машины, которые клиент
	заказывал в этот промежуток.

Функция ``` cars_of_client_at_moment``` принимает ID клиента, а также дату, и
	возвращает машины, которые арестовывал клиент в эту дату.

#raw(read("./req6.sql"), block: true, lang: "sql")

Чтобы получить автомобили, заказанные Пятковым Алексеем Морковичем, можно выполнить простой
	запрос:

#raw(read("./req6_show-1.sql"), block: true, lang: "sql")

Результат выполнения:

#figure(
	kind: table,
	caption: [Автомобили, заказанные Пятковым Алексеем Морковичем.],
	tablex(
		columns: (auto, auto),
		..csv("./res6_show-1.csv").flatten()
	)
)
...
#raw(read("./req6_show-2.sql"), block: true, lang: "sql")

#v(1cm)
#figure(
	kind: table,
	caption: [Автомобили, заказанные Красновым Дмитрием Александровичем с '2022-1-1' по '2023-1-1'.],
	tablex(
		columns: (auto, auto),
		..csv("res6_show-2.csv").flatten()
	)
)

Запрос, чтобы получить автомобили, заказанные Красновым Дмитрием
	Александровичем 2022-11-02.

#raw(read("./req6_show-3.sql"), block: true, lang: "sql")
...
В ходе проекта была разработана и внедрена база данных для управления компанией по выдачи автомобилей на прокат. На различных этапах проекта основное внимание уделялось организации данных, их целостности и возможностям анализа.
